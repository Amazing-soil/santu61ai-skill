...
AI 科技中文口播短视频的端到端剪辑 skill。输入口播视频和文案后，按真实语音时间线规划素材，混合 AI 图片、React/Remotion/Anime.js 动效和实操素材，产出最终高码率 MP4、实际使用的图片素材和预览检查图；可编辑 Remotion 工程默认只作为临时渲染工作区。
...
- 规划素材类型：实操录屏/截图、React/Remotion/Anime.js 动效、GPT Image/AI 图片、无素材过渡。
- 生成或整理素材，并统一放进工程媒体目录。
- 创建临时可编辑 Remotion 工程，核心配置写入 `src/timeline.json`。
- 按需生成可编辑字幕轨，字幕以真实语音口述为准。
- 渲染最终高码率 MP4，并用 `ffprobe`、关键帧抽帧图做验收。
- 验收通过后整理干净交付区，只保留最终视频、图片素材和预览检查图，按规则清理中间工程文件。
...
文案：……
要求：混合 AI 图片和 React 动效，导出 1080x1920 高码率 MP4，并保留图片素材和预览检查图。
```
...
`work/` 默认是临时工程；除非用户要求保留可编辑工程，最终只交付 `deliverables/final`、`deliverables/images`、`deliverables/previews`。
